...
- **`_available()` on every native extension** (`gpio`, `mem`, `store`, `wifi`, `s3_onboard_rgb`,
  `baremetal_utility`) returns a bool.
- **`gpio_delay()` is canonical; `delay()` is a plain alias** (no deprecation warning).
- **`bm_*` are canonical** (`bm_psram_free()`, `bm_heap_free()`, …); the unprefixed `psram_*` / `heap_*`
  names still work but are **deprecated** (the test confirms an `E_DEPRECATED` is emitted).
