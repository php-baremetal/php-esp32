...
| [`symfony-demo/`](symfony-demo/) | **Symfony 7.4** (skeleton + a controller) on the microSD, browsable over HTTP at ~2.1 s/request (prod, OPcache warm). A minimal, XML-free slice: Symfony's `ext-iconv`/`ext-xml` requirements are bypassed (a basic page doesn't call them). | `esp32-p4-eth` + network, `ctype`/`mbstring`/`tokenizer`/`session`/`date`/`opcache` + microSD |
