...
Memory is read with the `baremetal_utility` extension (`sys_psram_free()`, `sys_heap_free()`, ...). On this
...
1. **Engine baseline** — `sys_psram_free()` right after boot: how much PSRAM the running engine already
